...
import touchio
from macro import macroM



# create Mac object split into keyMac and TouchMac
Mac = macroM()
keyMac = Mac.getKeyMac()
touchMac = Mac.getTouchMac()
...
while True:
    ...
    if not button.value and button_state:
        ...
        if isinstance(keyMac[1], (list, tuple)) and isinstance(key_output[0], dict):
            for k in keyMac[1]:
                make_keystrokes(k['keys'], k['delay'])
        else:
            make_keystrokes(keyMac[1], delay=0)
        button_state = False
        time.sleep(0.2)

    if touch.value and not touch_state:
        print("Touched!")
        pixel.fill(touchMac[0])
        touch_state = True
    if not touch.value and touch_state:
        print("Untouched!")
        if isinstance(touchMac[1], (list, tuple)) and isinstance(touch_output[0], dict):
            for k in touchMac[1]:
                make_keystrokes(k['keys'], k['delay'])
        else:
            make_keystrokes(touchMac[1], delay=0)
        pixel.fill(0x0)
        touch_state = False
        time.sleep(0.2) # a bit of sleep to prevent rapid repeats
